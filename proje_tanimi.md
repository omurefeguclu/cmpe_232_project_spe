...
- **CandidateSkills (M:N Association):** Aday ile Yetenek arasındaki M:N ilişkiyi çözen ara tablodur. Adayın her yetenek için sahip olduğu seviyeyi (`ProficiencyLevel`: Beginner → Expert) kaydeder.
- **ForeignLanguage:** Sistemde tanımlı yabancı diller tablosudur (İngilizce, Almanca, Fransızca vb.).
- **CandidateForeignLanguages (M:N Association):** Aday ile Yabancı Dil arasındaki M:N ilişkiyi çözen ara tablodur. Adayın her dildeki yetkinlik seviyesini (`ProficiencyLevel`: A1 → C2) kaydeder.
...
### Mülakat Değerlendirmesi (Interviews — Aggregation Bağlantısı)
Projenin kalbini oluşturan bu yapı, **JobApplication** aggregation'ı ile **InterviewerUser** arasındaki ilişkidir. JobApplication zaten Candidate↔JobPosting ilişkisini bir varlık gibi temsil ettiği için, Interviews bu aggregation'ı referans alarak üçüncü bir varlık (InterviewerUser) ile bağlar. Bir Durum Makinesi (State Machine) mantığıyla (*Scheduled → Completed / Cancelled*) ilerler. Mülakatçı, `Score` ve `Notes` alanlarıyla adaya dair değerlendirmesini kaydeder.
...
| **Weak Entity** | `JobPosting` (Company'ye bağımlı) |
| **Aggregation** | `JobApplication` (Candidate↔JobPosting M:N → kutulanıp Interviews ilişkisine katılır) |
| **M:N Association** | `CandidateSkills`, `CandidateForeignLanguages` |
| **Aggregation Bağlantısı** | `Interviews` (JobApplication aggregation'ı + InterviewerUser) |
